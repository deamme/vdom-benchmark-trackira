...
(function (global, factory) {
   ...
   * @private
   */
  var removeNodeFromDom = function removeNodeFromDom(node) {
    if (node.parentNode) {
  ...
  Text.prototype = {
    ...
    destroy: function destroy() {
      // if the text node has been rendered we ensure it is removed from the DOM
      removeNodeFromDom(this.node);
    },
  ...
  Comment.prototype = {
     ...
     */
    destroy: function destroy() {
      // if the comment node has been rendered we ensure it is removed from the DOM
      removeNodeFromDom(this.node);
    },
      ...
      if (this.children.length === 1 && children[0]) {
        ...
        each(children, function (child) {
          if (child) {
            ...
            node.appendChild(child.render(this));
          }
        });
      }
    }
  ...
  var patch = function patch(container, fromChildren, toChildren) {
    ...
    if (toChildren.length === 0) {
      ...
      while (index < fromChildren.length) {
        fromChildren[index++].destroy();
      }
    } else {

      if (fromChildren.length === 1 && toChildren.length === 1) {

        var from = fromChildren[0],
            to = toChildren[0];

        // Implicit key with same type or explicit key with same key.
        if (from.key == null && from.equalTo(to) || from.key != null && from.key === to.key) {
          from.patch(to);
        } else {

          from.detach();
          container.appendChild(to.render());
        }
      } else {
        ...
        if (fromStartIndex > fromEndIndex) {
          ...
          for (; toStartIndex <= toEndIndex; toStartIndex++) {
            container.insertBefore(toChildren[toStartIndex].render(), before);
          }
  ...
  var prototype_patch = function prototype_patch(ref) {
    ...
    // Update properties
    if (this.props !== ref.props) {
      patchProperties(node, ref.props, this.props);
    }
    // Update attributes
    if (this.attrs !== ref.attrs) {
  ...
  var init = function init(tagName, options, children) {
    ...
    /**
     * Properties to be applied to the created DOM node.
     */
    this.props = options.props;

    /**
     * Attributes to be applied to the created DOM node.
     */
