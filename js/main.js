...
(function (global, factory) {
    ...
    var normalize = function normalize(nodes, parent) {
    ...
    var attach = function attach(root, nodes, parent) {
        ...
        while (i < nodesLen) {
            ...
            if (nodes[i]) {
                ...
                // Virtual text -and comment nodes are more or less equal
                if (nodes[i].type === types__COMMENT || nodes[i].type === types__TEXT) {
    ...
    var toHTML = function toHTML() {
        ...
        // create markup for attributes
        if (node.attrs) {

            html += createMarkupForAttributes(node.attrs, node.tagName);
        }

        // create markup for properties
        if (node.props) {
        ...
        return html += selfClosing[tagName] ? '' : '</' + tagName + '>';
    };
    ...
    var renderStyles = function renderStyles(node, key, value) {
        ...
        for (key in attrs) {
            ...
            if (val !== undefined) {
                ...
                switch (key) {
                    ...
                    default:
                        // namespace attributes
                        if (special_namespace.set[key]) {

                            special_namespace.set[key](key, node, attrs, {});
                        } else if (val != null) {
    ...
    var renderSelect = function renderSelect(node) {
        ...
        if (selectValue !== undefined) {
            ...
            if (isArray(selectValue)) {
                ...
                while (index < selectValue.length) {
    ...
    var prototype_destroy = function prototype_destroy() {
        ...
        if (node) {
            ...
            if (parentElement = node.parentElement) {
                ...
                this.hooks.destroy(node, function () {
    ...
    var keyIndexes = function keyIndexes(fromChildren, startIndex, endIndex) {
        var index,
            keys = {};

        for (index = endIndex; index >= startIndex; index--) {
            if (fromChildren[index].key !== undefined) {
                keys[fromChildren[index].key] = index;
            }
        }
    ...
    var patch = function patch(container, fromChildren, toChildren) {

        var index = 0;

        if (fromChildren.length === 0) {

            while (index < toChildren.length) {
                toChildren[index++].destroy();
            }
        } else {
            ...
            while (fromStartIndex <= fromEndIndex && toStartIndex <= toEndIndex) {
                ...
                if (fromStartNode === undefined) {
                    ...
                    if (ChildrenMap === undefined) {
                        ChildrenMap = keyIndexes(fromChildren, fromStartIndex, fromEndIndex);
                    }
            ...
            if (fromStartIndex > fromEndIndex) {
                ...
                for (; toStartIndex <= toEndIndex; toStartIndex++) {
                    if (toChildren[toEndIndex + 1] === undefined) {
                        container.appendChild(toChildren[toStartIndex].render());
                    } else {
                        container.insertBefore(toChildren[toStartIndex].render(), toChildren[toEndIndex + 1].node);
                    }
                }
    ...
    var patchProperties = function patchProperties(node, props, prevProps) {
        ...
        for (key in prevProps) {
            ...
            if (prevProps[key] === undefined || props[key] !== undefined) {
                continue;
            }
        ...
        for (key in props) {
            ...
            if (key === 'className') {
                ...
                node.className = props[key] ? processClasses(props[key]) : '';
            } else if (prevProps[key] !== props[key]) {
    ...
    var patchAttributes = function patchAttributes(node, attrs, previousAttr) {
        ...
        var styleName, attrName, previousAttrValue, attrValue;

        for (attrName in previousAttr) {

            previousAttrValue = previousAttr[attrName];

            if (!previousAttrValue || attrs[attrName] != null) {
                continue;
            }
            ...
            if (attrName === 'style') {
                ...
                // namespace attributes
            } else if (special_namespace.unset[attrName]) {

                special_namespace.unset[attrName](attrName, node, previousAttr);
            } else {
    ...
    var prototype_patch = function prototype_patch(ref) {
        ...
        if (this.equalTo(ref)) {
            ...
            // Update children
            patch(node, this.children, ref.children);
    ...
    var detach = function detach(destroy) {
        ...
        if (this.hooks && this.hooks.detach) {

            this.hooks.detach(this, this.node);
        }
    ...
    var update = function update(root, prevNodes, nodes, parent) {
        return patch(root, prevNodes, normalize(nodes), parent);
    };
    ...
    var apply = function apply(selector, factory, container, children) {
        ...
        if (root) {
            ...
            mountId = container.mountId ? container.mountId : this.guid();
    ...
    var prototype_children = function prototype_children(mountId) {
    ...
    var mountPoint = function mountPoint(mountId) {
